...
describe('RootLayout CSP integration', () => {
  it('uses self-hosted-first root metadata without trial or hosted pricing claims', () => {
    ...
    expect(metadata.description).toContain('Run Canary Wallet on your Bitcoin node')
  ...
  it('adds the request nonce to every inline script in the document head', async () => {
    const layout = await RootLayout({ children: <main>Canary Wallet</main> })
    ...
    expect(scripts[0].props.dangerouslySetInnerHTML.__html).toContain('canary-wallet-theme')
