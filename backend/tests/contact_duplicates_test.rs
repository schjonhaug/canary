use canary::{
    ...
    metadata::{MetadataDb, ProviderType, Language},
};
use std::sync::Arc;
...
async fn test_duplicate_email_prevention() {
    ...
    let metadata_db = Arc::new(MetadataDb::new(db_path.to_str().unwrap(), &config).await.unwrap());
    
    // Create a test user
    let user_id = metadata_db
        .create_user("test@example.com", "hash", Some("Test User"), false)
        .await
        .unwrap();
    ...
    let duplicates = metadata_db
        .check_duplicate_notification_targets(&wallet_checksum, &[
    ...
    assert!(duplicates[0].contains("john@example.com"), "Error message should mention the duplicate email");
    assert!(duplicates[0].contains("John"), "Error message should mention the existing contact name");
    ...
    assert!(!case_duplicates.is_empty(), "Should detect case-insensitive duplicate email");
    ...
    let cross_wallet_duplicates = metadata_db
        .check_duplicate_notification_targets(&wallet2_checksum, &[
            ("email".to_string(), "john@example.com".to_string())
        ], None)
        .await
        .unwrap();
    
    assert!(cross_wallet_duplicates.is_empty(), "Same email should be allowed in different wallet");
    ...
    assert!(self_update_duplicates.is_empty(), "Contact should be able to keep its own email on update");
}
...
async fn test_duplicate_phone_prevention() {
    ...
    assert!(!duplicates.is_empty(), "Should detect duplicate phone number");
    assert!(duplicates[0].contains("+4712345678"), "Error message should mention the duplicate phone");
    assert!(duplicates[0].contains("Alice"), "Error message should mention the existing contact name");
    ...
    assert!(cross_wallet_duplicates.is_empty(), "Same phone should be allowed in different wallet");
    ...
    assert!(self_update_duplicates.is_empty(), "Contact should be able to keep its own phone on update");
}
...
async fn test_ntfy_topics_excluded_from_duplicate_check() {
    ...
    assert!(duplicates.is_empty(), "ntfy topics should be excluded from duplicate checking");
}
...
async fn test_mixed_provider_types_allowed() {
    ...
    assert!(duplicates.is_empty(), "Same string should be allowed for different provider types");
}
...
async fn test_verification_endpoint_rejects_duplicates() {
    ...
    let duplicate_check_result = metadata_db
        .check_duplicate_notification_target(&wallet_checksum, "email", "existing@example.com", None)
        .await
        .unwrap();
    
    assert!(duplicate_check_result.is_some(), "Should detect duplicate email during verification");
    assert_eq!(duplicate_check_result.unwrap(), "Existing Contact", "Should return the existing contact name");
    ...
    assert!(no_duplicate_result.is_none(), "Should not detect duplicate for different email");
}