...
impl WalletManager {
    ...
    /// Get wallet balance without triggering a full sync
    /// Used for rate-triggered alert checking
    /// Returns None if wallet is not loaded in memory
    pub async fn get_wallet_balance(&self, checksum: &str) -> Result<Option<i64>, anyhow::Error> {
        let wallets_map = self.wallets.lock().await;

        // Check if wallet is loaded in memory
        if let Some(wallet_mutex) = wallets_map.get(checksum) {
            let wallet_guard = wallet_mutex.lock().await;
            let (wallet, _) = &*wallet_guard;

            // Get current balance
            let balance = wallet.balance();
            Ok(Some(balance.total().to_sat() as i64))
        } else {
            // Wallet not loaded in memory - skip
            Ok(None)
        }
    }

    /// Check balance alerts for wallets with fiat thresholds after exchange rate changes
    /// This is called after exchange rates are refreshed (every 10 minutes)
    /// Only checks wallets that have active fiat alerts and are loaded in memory
    pub async fn check_alerts_for_rate_changes(&self) -> Result<()> {
        use crate::sync::WalletSyncService;

        debug!("Checking balance alerts for exchange rate changes");

        // Get all wallets with active fiat alerts
        let wallet_checksums = self
            .metadata_db
            .get_wallets_with_active_fiat_alerts()
            .await?;

        if wallet_checksums.is_empty() {
            debug!("No wallets with active fiat alerts");
            return Ok(());
        }

        debug!(
            "Found {} wallet(s) with active fiat alerts",
            wallet_checksums.len()
        );

        // Create sync service for alert checking
        ...
        for wallet_checksum in wallet_checksums {
            // Get wallet balance without triggering full sync
            match self.get_wallet_balance(&wallet_checksum).await {
                Ok(Some(balance_sats)) => {
                    debug!(
                        "[{}] Checking fiat alerts after rate change (balance: {} sats)",
                        wallet_checksum, balance_sats
                    );

                    // Check balance alerts with current balance
                    if let Err(e) = sync_service
                        .check_balance_alerts(&wallet_checksum, balance_sats)
                        .await
                    {
                        warn!(
                            "[{}] Failed to check balance alerts after rate change: {}",
                            wallet_checksum, e
                        );
                    } else {
                        checked_count += 1;
                ...
                Ok(None) => {
                    // Wallet not loaded in memory - will be checked on next regular sync
                    debug!(
                        "[{}] Wallet not loaded in memory, skipping rate-triggered check",
                        wallet_checksum
                    );
                    skipped_count += 1;
                }
                Err(e) => {
                    warn!(
                        "[{}] Failed to get wallet balance for rate-triggered check: {}",
                        wallet_checksum, e
                    );
                    skipped_count += 1;
                }
        ...
        if checked_count > 0 {
            info!(
                "Rate-triggered alert check completed: {} checked, {} skipped",
