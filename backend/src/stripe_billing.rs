...
impl StripeBilling {
    ...
    pub async fn create_trial_subscription(
        ...
        let tier_pricing = self
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "No pricing found for tier {:?}. Make sure products are loaded from Stripe.",
                    tier
                )
            })?;
        ...
        let price_info = tier_pricing
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "No price found for tier {:?}. Make sure products have prices in Stripe.",
                    tier
                )
            })?;
        ...
        tracing::info!(
            "🆕 Creating trial subscription for user {} with tier {:?} (price: {})",
            user.email,
            tier,
            price_id
        );
        ...
        metadata_db
            .update_user_stripe_customer(&user.id, &customer.id.unwrap_or_default())
            .await?;

        tracing::info!(
            "✅ Trial subscription created successfully for user {}",
            user.email
        );
        Ok(())
    }
