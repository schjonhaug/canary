...
use crate::stripe_client_service::{StripeClientService, Customer, Subscription, CheckoutSession, BillingPortalSession, Product, Price, Coupon};

/// Stripe billing configuration and client
pub struct StripeBilling {
    client: StripeClientService,
    webhook_secret: String,
    products: HashMap<SubscriptionTier, String>, // Use String instead of ProductId
    monthly_prices: HashMap<SubscriptionTier, String>, // Use String instead of PriceId
    yearly_prices: HashMap<SubscriptionTier, String>, // Use String instead of PriceId
        ...
        for product in products {
            ...
            if let Some(metadata) = &product.metadata {
                if let Some(tier_str) = metadata.get("tier") {
                    ...
                    self.products.insert(tier, product_id.clone());
                    ...
                    for price in prices {
                        if let Some(recurring) = &price.recurring {
                            ...
                            match interval {
                                "month" if interval_count == 1 => {
                                    self.monthly_prices.insert(tier, price.id.clone().unwrap_or_default());
                                ...
                                "year" if interval_count == 1 => {
                                    self.yearly_prices.insert(tier, price.id.clone().unwrap_or_default());
                                    let amount = price.unit_amount.unwrap_or(0);
                                    let currency = price.currency.as_deref().unwrap_or("USD");
