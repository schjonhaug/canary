...
async fn test_alice_partial_send_bob_two_stage() {
    ...
    let unconfirmed_alice_transactions = env
        .get_wallet_transactions(&env.alice_checksum)
        .await
        .expect("Failed to get Alice transactions");
    let unconfirmed_bob_transactions = env
        .get_wallet_transactions(&env.bob_checksum)
        .await
        .expect("Failed to get Bob transactions");
    ...
    println!(
        "   Initial Alice transactions: {}",
        initial_alice_transactions.len()
    );
    println!(
        "   Total Alice transactions: {}",
        unconfirmed_alice_transactions.len()
    );
    for (i, transaction) in unconfirmed_alice_transactions.iter().enumerate() {
        let is_new = i >= initial_alice_transactions.len();
        println!(
            "   Transaction {}: type={:?}, amount={}, confirmed={}, NEW={}",
            i,
            transaction.transaction_type,
            transaction.amount_sats,
            transaction.block_height.is_some(),
            is_new
        );
    }
    ...
    let initial_alice_ids: Vec<String> = initial_alice_transactions
        .iter()
        .map(|t| t.txid.clone())
        .collect();
    let initial_bob_ids: Vec<String> = initial_bob_transactions
        .iter()
        .map(|t| t.txid.clone())
        .collect();
    ...
    println!(
        "   Alice unconfirmed sends: {}",
        alice_unconfirmed_sends.len()
    );

    assert_eq!(
        alice_unconfirmed_sends.len(),
        1,
        "Alice should have exactly one unconfirmed Send event"
    );
    assert_eq!(
        bob_unconfirmed_receives.len(),
        1,
        "Bob should have exactly one unconfirmed Receive event"
    );
    ...
    let final_alice_transactions = env
        ...
        .await
        .expect("Failed to get Alice transactions");
    let final_bob_transactions = env
        .get_wallet_transactions(&env.bob_checksum)
        .await
        .expect("Failed to get Bob transactions");

    let alice_confirmed_sends: Vec<_> = final_alice_transactions
        .iter()
        .filter(|t| t.transaction_type == EventType::Send && t.block_height.is_some())
        .collect();
    let bob_confirmed_receives: Vec<_> = final_bob_transactions
        .iter()
        .filter(|t| t.transaction_type == EventType::Receive && t.block_height.is_some())
        .collect();

    assert!(
        !alice_confirmed_sends.is_empty(),
        "Alice should have confirmed Send event after mining"
    );
    assert!(
        !bob_confirmed_receives.is_empty(),
        "Bob should have confirmed Receive event after mining"
    );

    // Verify amount is correct (0.1 BTC = 10M sats)
    let expected_amount = 10_000_000i64;
    let bob_received_correct_amount = bob_confirmed_receives
        .iter()
        .any(|t| t.amount_sats == expected_amount);

    assert!(
        bob_received_correct_amount,
        "Bob should receive 0.1 BTC (10M sats)"
    );
...
async fn test_alice_full_send_bob_two_stage() {
    ...
    let _txid = env
        .send_transaction("alice", "bob", "max")
        .await
        .expect("Failed to send max transaction");
    ...
    assert_eq!(
        alice_unconfirmed_sends.len(),
        1,
        "Alice should have exactly one unconfirmed Send event for full send"
    );
    assert_eq!(
        bob_unconfirmed_receives.len(),
        1,
    ...
    assert!(
        alice_full_send_amount > 50_000_000,
        "Alice should be sending large amount (> 0.5 BTC), got: {} sats",
        alice_full_send_amount
    );
    ...
    let alice_confirmed_large_sends: Vec<_> = alice_confirmed_sends
        .iter()
        .filter(|t| t.amount_sats.abs() > 50_000_000) // > 0.5 BTC
        .collect();
    let bob_confirmed_large_receives: Vec<_> = bob_confirmed_receives
        .iter()
        .filter(|t| t.amount_sats > 50_000_000) // > 0.5 BTC
        .collect();

    assert!(
        !alice_confirmed_large_sends.is_empty(),
        "Alice should have large confirmed Send transactions"
    );
    assert!(
        !bob_confirmed_large_receives.is_empty(),
        "Bob should receive large amounts from full send"
    );
...
async fn test_multiple_partial_sends_bob_two_stage() {
    ...
    let _txid1 = env
        .send_transaction("alice", "bob", "0.1")
        .await
        .expect("Failed to send transaction 1");
    let _txid2 = env
        .send_transaction("alice", "bob", "0.2")
        .await
        .expect("Failed to send transaction 2");
    let _txid3 = env
        .send_transaction("alice", "bob", "0.05")
        .await
        .expect("Failed to send transaction 3");
    ...
    assert_eq!(
        alice_unconfirmed_sends.len(),
        3,
        "Alice should have exactly 3 unconfirmed Send events"
    );
    assert_eq!(
        bob_unconfirmed_receives.len(),
        3,
        "Bob should have exactly 3 unconfirmed Receive events"
    );
    ...
    let mut alice_amounts: Vec<i64> = alice_unconfirmed_sends
        .iter()
        .map(|t| t.amount_sats.abs())
        .collect();
    let mut bob_amounts: Vec<i64> = bob_unconfirmed_receives
        .iter()
        .map(|t| t.amount_sats)
        .collect();
    ...
    for (i, transaction) in bob_confirmed_receives.iter().enumerate() {
        println!(
            "   Bob Confirmed Receive {}: {} sats",
            i, transaction.amount_sats
        );
    }

    assert!(
        !alice_confirmed_sends.is_empty(),
        "Alice should have confirmed Send events after mining"
    );
    assert!(
        !bob_confirmed_receives.is_empty(),
